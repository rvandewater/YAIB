...
class SimpleDiffusionModel(ImputationWrapper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Quadratic
            ...
            self.betas = (
                torch.linspace(self.min_noise**0.5, self.max_noise**0.5, self.T) ** 2
            )
        # Cosine
        elif self.noise_scheduler == "cosine":
            x = torch.linspace(0, self.T, self.T + 1)
            alphas_cumprod = (
                torch.cos(((x / self.T) + 0.008) / (1 + 0.008) * torch.pi * 0.5) ** 2
            )
        ...
        # Sigmoid
            ...
            self.betas = (
                torch.sigmoid(betas) * (self.max_noise - self.min_noise)
                + self.min_noise
            )
        # Error
        else:
        ...
        for i in range(self.n_onedirectional_conv):
            ...
            self.ups.append(
                Block(input_size, (self.n_onedirectional_conv - i), up=True)
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        for metric in self.metrics["test"].values():
            metric.update(
                (torch.flatten(x_0, start_dim=1), torch.flatten(target, start_dim=1))
            )
