...
We refer to the page `adding a new model <https://github.com/rvandewater/YAIB/wiki/Adding-a-new-model>`_ for detailed instructions on adding new models.
...
built-in parallelism. Our interface derives from the `BaseModule <https://lightning.ai/docs/pytorch/stable/common/lightning_module.html>`_.
...
.. code-block:: python
   :caption: Example ML model definition
...
.. code-block:: python
   :caption: Example DL model definition
...
* *Do you want to manually define the model or use an existing library?* This might require adapting the ``DLPredictionWrapper``.
* *Does the model expect the data to be in a certain format?* This might require adapting the ``PredictionDataset``.
...
.. code-block:: python
   :caption: Example building block
...
The next step is to use the building blocks defined in ``layers.py`` or modules from an existing library to add to the model in ``models/dl_models.py``. In this case, we use the Pytorch-forecasting library (https://github.com/jdb78/pytorch-forecasting):
...
.. code-block:: python
   :caption: Example custom dataset definition
